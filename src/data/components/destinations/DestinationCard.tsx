...
export default function DestinationCard({ destination, onWishlist, isWishlisted }: Props) {
  return (
    <motion.div
      ...
      className="group bg-white rounded-3xl overflow-hidden card-shadow hover:shadow-elevated relative"
    >
        ...
        <div className="relative h-52 overflow-hidden">
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
            ...
            <span className="glass-dark text-white text-xs font-bold px-3 py-1 rounded-full">
              {formatPrice(destination.price)}/day
            </span>
          </div>
        </div>
        ...
        <div className="p-5">
          <div className="flex items-center gap-1 mb-2">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`w-3.5 h-3.5 ${i < Math.round(destination.rating) ? 'fill-accent-400 text-accent-400' : 'text-dark-200'}`}
              />
            ))}
            <span className="text-xs text-dark-400 ml-1 font-medium">{destination.rating.toFixed(1)}</span>
            <span className="text-xs text-dark-300">({destination.reviewCount})</span>
          </div>
